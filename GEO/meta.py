...
def metaAnalysis(settingFile):
    ...
    if metaMethod == 'MAMA':
        ...
        s += 'write.table(rownames(merged), file="{}/geneOrder.tsv", sep="\t")'\
            .format(metaDir)
...
def meta2Csv(settingFile):
    ...
    if not settings['mapping to gene in meta'] \
        ...
        gpls = set(gpls)
        rfs = [open('{}/{}_probe2symbol.json'.format(annotDir, gpl), 'r')
               for gpl in gpls]
    ...
    if metaMethod == 'MAMA':
        pMatrix = pd.read_csv('{}/p_value'.format(metaDir), sep='\t')
        esMatrix = pd.read_csv('{}/es'.format(metaDir), sep='\t')
        # merged = pd.concat([pMatrix, esMatrix], axis=1, join='inner')
        merged = pMatrix.join(esMatrix, how='inner', lsuffix='_p', rsuffix='_es')
        # merged = merged[['c_pval', 'zSco']]
        if not settings['mapping to gene in meta'] \
                and settings['probe mapping to gene']:
            annot = pd.DataFrame.from_dict(probe2symbol, orient='index',
                                           columns=['symbol'])
            # merged = pd.concat([annot, merged], axis=1, join='inner')
            merged = merged.join(annot, how='left')
        merged.to_excel('{}/meta_results.xlsx'.format(metaDir))
        for p in settings['p-value cutoff']:
            for es in settings['effect size cutoff']:
                selected = merged[(merged['FDR_p'] < p)
                                  & (abs(merged['zSco']) > es)]
                selected.to_excel('{}/{}_{}.xlsx'.format(metaDir, p, es))
